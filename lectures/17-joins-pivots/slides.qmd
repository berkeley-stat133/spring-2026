...
How do you go from `dat1` to `dat2`?

```{r}
#| echo: false
dat1 <- tibble(
  name = c("Harry", "Hermione", "Ron"),
  fall = c("spells-1A", "potions-1B", "charms-1A"),
  spring = c("potions-1A", "spells-1B", "charms-1B"))
...
tb3 <- tb1 |>
  mutate(`spells/potions` = spells / potions) |>
  select(name, year, `spells/potions`)
```
...
```{r}
#| eval: false
#| code-line-numbers: "1|1-2|1-3|1-4|1-5|1-6"
tb1 <- separate(
  data = tb4,
  col = ratio,
  into = c("spells", "potions"),
  sep = "/", 
  remove = TRUE)
```
